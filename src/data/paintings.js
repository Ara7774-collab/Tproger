...
export const locations = [...new Set(paintings.map(p => p.location))].sort();
export const minYear = Math.min(...paintings.map(p => p.year));
export const maxYear = Math.max(...paintings.map(p => p.year));